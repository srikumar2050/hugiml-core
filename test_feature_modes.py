...
def test_save_load_preserves_feature_mode_and_predictions(mode, tmp_path):
    ...
    # Hybrid modes may restore a StandardScaler fitted on ndarray-backed features;
    # sklearn warns when the caller supplies a DataFrame with feature names.
    # The test asserts prediction equality, so suppress only this expected warning.
