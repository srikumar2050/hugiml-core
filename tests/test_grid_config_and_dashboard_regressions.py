...
"""Grid-configuration and dashboard-wiring regression checks.

Covers native categorical-extraction consistency, zero-variance column
exclusion, the recommended named hyperparameter grids (including the
"performance_ho" default and its RPTE branch), tuning execution_mode
threading, and two dashboard regressions (the Governance cold-start message
and the Workbench tuning-grid selector).

Run directly:

    python3 test_grid_config_and_dashboard_regressions.py

or under pytest:

    pytest test_grid_config_and_dashboard_regressions.py -v
...
def run_grid_config_section() -> None:
    ...
    _report(
        "default grid name is 'performance_ho'", DEFAULT_HUGIML_GRID_NAME == "performance_ho"
    )
    _report(
    ...
    _report(
        "performance_ho grid searches both LR (None) and an RPTE base_estimator",
        get_hugiml_grid("performance_ho").get("base_estimator", [])
        and get_hugiml_grid("performance_ho")["base_estimator"][0] is None
        and get_hugiml_grid("performance_ho")["base_estimator"][1] is not None,
    )
    ...
    _report(
        "interpretability grid treats numeric binary indicators categorically",
        interp.get("convert_binary_to_categorical") == [True],
    )
    _report(
...
def run_workbench_grid_selector_section() -> None:
    ...
    try:
        ...
        if selectors:
            ...
            _report(
                "tuning-grid selector offers all four named grids",
...
def test_hugiml_named_grids_have_expected_values_and_are_copy_safe() -> None:
    ...
    assert {key: value for key, value in performance.items() if key != "base_estimator"} == {
        "B": [-1],
        "adaptive_binning": [True],
        "L": [1, 2],
        "topK": [50, 100],
    ...
    assert len(performance["base_estimator"]) == 1
    linear = performance["base_estimator"][0]
    assert linear.solver == "liblinear"
    linear_params = linear.get_params()
    assert linear_params.get("penalty") == "l1" or linear_params.get("l1_ratio") == 1.0
    ...
    interpretability_ho = get_hugiml_grid("interpretability_ho")
    assert len(list(ParameterGrid(interpretability_ho))) == 16
    assert interpretability_ho["feature_mode"] == ["patterns_only"]
    assert interpretability_ho["interaction_relaxed_mining"] == [True]
    assert interpretability_ho["augmented_pair_transforms"] == [False]
    assert interpretability_ho["convert_binary_to_categorical"] == [True]
    assert interpretability_ho["topk_budget_strict"] == [False]
    assert len(interpretability_ho["base_estimator"]) == 2
    assert interpretability_ho["base_estimator"][1].estimator.enable_lookahead is False

    performance_ho = get_hugiml_grid("performance_ho")
    assert len(list(ParameterGrid(performance_ho))) == 16
    assert performance_ho["topk_budget_strict"] == [False]
    assert performance_ho["convert_binary_to_categorical"] == [False]
    # get_hugiml_grid must deep-copy: performance_ho holds live sklearn
    # estimator instances (base_estimator), and a shallow {k: list(v)} copy
    # (the pre-RPTE implementation) would let a caller's mutation of one of
    # those estimator objects leak into the process-wide grid definition.
    performance_ho_again = get_hugiml_grid("performance_ho")
    assert performance_ho["base_estimator"][1] is not performance_ho_again["base_estimator"][1]
...
def test_classifier_default_param_grid_accepts_named_grid() -> None:
    ...
    for key in default_grid:
        if key == "base_estimator":
            # Estimator instances aren't equality-comparable in a meaningful
    ...
    for grid_name in ("performance", "interpretability"):
        classifier_grid = HUGIMLClassifier.default_param_grid(grid_name)
        named_grid = get_hugiml_grid(grid_name)
        assert classifier_grid.keys() == named_grid.keys()
        for key in classifier_grid:
            if key == "base_estimator":
                assert len(classifier_grid[key]) == len(named_grid[key]) == 1
                assert classifier_grid[key][0].get_params() == named_grid[key][0].get_params()
                assert classifier_grid[key][0] is not named_grid[key][0]
            else:
...
def test_benchmark_dashboard_scenarios_keep_existing_selector_ids() -> None:
    import ast
    ...
    for node in module.body:
        if not isinstance(node, ast.AnnAssign):
            continue
        if isinstance(node.target, ast.Name) and node.target.id == "HUGIML_SCENARIOS":
...
def test_script_entrypoint_is_available() -> None:
