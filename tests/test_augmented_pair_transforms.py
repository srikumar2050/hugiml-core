...
def test_augmented_pair_transforms_are_persisted_for_l2_and_capped_by_topk():
    ...
    assert all(t["aug_feature_size"] == 10 for t in transforms)
    assert [t["rank"] for t in transforms] == list(range(1, len(transforms) + 1))


def test_aug_feature_size_controls_candidate_pool():
    ...
    assert all(t["aug_feature_size"] == 5 for t in transforms)
...
def test_augmented_pair_params_follow_sklearn_get_set_params():
    ...
    assert params["aug_feature_size"] == 10
    model.set_params(augmented_pair_transforms=False, aug_feature_size=5)
    assert model.augmented_pair_transforms is False
    assert model.aug_feature_size == 5


def test_augmented_pair_serialization_pickle_roundtrip():
    X, y = _interaction_data(n=100, p=8)
    model = HUGIMLClassifierNative(
        B=-1, adaptive_binning=True, L=2, topK=30, interaction_relaxed_mining=False
    )
    model.fit(X, y)
    restored = pickle.loads(pickle.dumps(model))
    assert restored.augmented_pair_transforms is True
    assert restored.aug_feature_size == 10
    assert restored.get_augmented_pair_transforms() == model.get_augmented_pair_transforms()


def test_augmented_pair_max_features_alias_sets_new_source_budgets():
    clf = HUGIMLClassifierNative(augmented_pair_max_features=6)

    assert clf.aug_feature_size == 6
    assert clf.max_pair_features == 6
    assert clf.get_params()["augmented_pair_max_features"] == 6


def test_augmented_pair_max_features_alias_preserves_explicit_new_budgets():
    ...
    assert clf.aug_feature_size == 8
    assert clf.max_pair_features == 9
    assert clf.augmented_pair_max_features == 6
