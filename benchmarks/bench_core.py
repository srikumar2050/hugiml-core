...
def _load_german():
    ...
    df = pd.read_csv(path, sep=" ", header=None, names=GERMAN_COLS + ["target"])
...
def bench_fit(clf_cls, X_tr, y_tr, label: str) -> dict:
    ...
    X_f, _, y_f, _ = train_test_split(X_p, y_p, test_size=0.1, random_state=0, stratify=y_p)
...
def main():
    ...
    parser.add_argument("--dataset", choices=["german", "heloc", "all"], default="all")
