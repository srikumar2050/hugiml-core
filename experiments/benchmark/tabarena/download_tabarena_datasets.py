...
DEFAULT_DATASETS_DIR = PROJECT_ROOT / "datasetsTabarena"
# Official source: autogluon/tabarena, TabArena-v0.1 curated metadata.
# Verified against upstream commit 36e736afbcc04be1256eddcfa28a695c0209b94b.
TABARENA_REGISTRY_URL = "https://github.com/autogluon/tabarena"
TABARENA_REGISTRY_REVISION = "36e736afbcc04be1256eddcfa28a695c0209b94b"
TABARENA_SUITE_ID = "TabArena-v0.1-classification"
TABARENA_CLASSIFICATION_TASK_IDS = [
    363613, 363614, 363616, 363618, 363619, 363620, 363621, 363623,
    363624, 363626, 363627, 363628, 363629, 363630, 363632, 363671,
    363673, 363674, 363676, 363677, 363679, 363681, 363682, 363683,
    363684, 363685, 363711, 363689, 363691, 363694, 363696, 363699,
    363700, 363702, 363704, 363706, 363707, 363712,
]
...
def tabarena_repeat_count(n_rows: int) -> int:
    """Return the official TabArena-v0.1 repeat budget for a dataset."""
    if n_rows < 2_500:
        return 10
    if n_rows > 250_000:
        return 1
...
def save_task(root: Path, selected: dict[str, object]) -> dict[str, object]:
    ...
    openml_repeats, folds, samples = (int(value) for value in task.get_split_dimensions())
    repeats = min(openml_repeats, tabarena_repeat_count(len(X)))
...
def select_download_tasks(
    ...
    *,
    ...
    if add is not None:
        ...
        return selected, f"complete_cache_plus_{len(additions)}_next_missing_tasks_in_official_order"
    target_count = 38 if smallest is None else int(smallest)
    target_count = min(target_count, len(ordered_tasks))
    return ordered_tasks[:target_count], f"first_{target_count}_tasks_in_official_order"


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Download TabArena datasets and official splits for offline benchmarking"
    )
    ...
    selection.add_argument(
        ...
        type=int,
        default=None,
        help="Target total number of TabArena classification tasks to cache in official order (default: all 38)",
    )
    selection.add_argument(
        "--add",
        type=int,
        default=None,
        help="Add this many next missing TabArena tasks in official order while retaining complete cached tasks",
    )
    parser.add_argument(
        "--refresh",
        action="store_true",
    ...
    suite = SimpleNamespace(tasks=TABARENA_CLASSIFICATION_TASK_IDS, id=TABARENA_SUITE_ID)
    ...
    def write_progress(*, complete: bool) -> None:
        manifest = {
            ...
            "suite": "TabArena",
            "suite_id": TABARENA_SUITE_ID,
            "registry_url": TABARENA_REGISTRY_URL,
            "registry_revision": TABARENA_REGISTRY_REVISION,
            "registry_subset": "is_classification == True",
