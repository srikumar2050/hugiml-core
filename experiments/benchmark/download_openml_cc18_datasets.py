...
def task_rows(suite: object) -> list[dict[str, object]]:
    ...
    if found != set(task_ids):
        ...
        raise RuntimeError(f"OpenML task listing omitted task IDs: {missing}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Download OpenML-CC18 datasets and official splits for offline benchmarking"
    )
    ...
    selection.add_argument(
        ...
        type=int,
        default=None,
        help="Target total number of smallest CC18 tasks to cache (default: 36)",
    )
    selection.add_argument(
        "--add",
        type=int,
        default=None,
        help="Add this many next-smallest missing CC18 tasks while retaining complete cached tasks",
    )
    parser.add_argument(
        "--refresh",
        action="store_true",
